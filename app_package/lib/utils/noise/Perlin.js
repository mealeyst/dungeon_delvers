...
function gradient4D(hash, x, y, z, t) {
    const H = hash & 31, U = H < 24 ? x : y, V = H < 16 ? y : z, W = H < 8 ? z : t;
 ...
 */
const perlin2D = (x, y) => {
    const FX = Math.floor(x), FY = Math.floor(y), X = FX & 255, Y = FY & 255, A = P[X] + Y, B = P[X + 1] + Y;
    x = x - FX;
    y = y - FY;
...
const perlin3D = (x, y, z) => {
    const FX = Math.floor(x), FY = Math.floor(y), FZ = Math.floor(z), X = FX & 255, Y = FY & 255, Z = FZ & 255, A = P[X] + Y, B = P[X + 1] + Y, AA = P[A] + Z, BA = P[B] + Z, AB = P[A + 1] + Z, BB = P[B + 1] + Z;
    x = x - FX;
    y = y - FY;
    z = z - FZ;
 ...
 * @returns {number} Perlin Noise value.
 */
const perlin4D = (x, y, z, t) => {
    const FX = Math.floor(x), FY = Math.floor(y), FZ = Math.floor(z), FT = Math.floor(t), X = FX & 255, Y = FY & 255, Z = FZ & 255, T = FT & 255, A = P[X] + Y, B = P[X + 1] + Y, AA = P[A] + Z, BA = P[B] + Z, AB = P[A + 1] + Z, BB = P[B + 1] + Z, AAA = P[AA] + T, BAA = P[BA] + T, ABA = P[AB] + T, BBA = P[BB] + T, AAB = P[AA + 1] + T, BAB = P[BA + 1] + T, ABB = P[AB + 1] + T, BBB = P[BB + 1] + T;
    x = x - FX;
    y = y - FY;
    z = z - FZ;
    t = t - FT;
...
PerlinWrapper.noise2D = (x, y) => {
    return (0, exports.perlin2D)(x, y) * 2.0 - 1.0;
};
exports.default = PerlinWrapper;
