...
export class Actor extends TransformNode {
  ...
  async load() {
    ...
    this._mesh.material.diffuseColor = this._npc
      ? new Color3(0, 0, 1)
      : new Color3(1, 0, 0)
    ...
    if (this._input) {
      if (this._input.inputMap['w']) {
        this._mesh.moveWithCollisions(
          this._mesh.forward.scale(this._forwardSpeed),
        )
      }
      if (this._input.inputMap['s']) {
        this._mesh.moveWithCollisions(
          this._mesh.forward.scale(-this._backwardSpeed),
        )
      }
